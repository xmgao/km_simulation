...
int main()
{
	...
	{
		// 处理读取失败的情况
		perror("Failed to read timestamp");
	}
	pre_ts = timestamp;
	...
	while (1) // 循环
	{
		...
		fread(block, sizeof(uint8_t), length, fp);
		packet.ConstrctPacket(seq, length + 4, block);
		...
		if ((nsendSize = write(i_connfd_1, packet.getBufferPtr(), packet.getBufferSize())) < 0)
		{
		...
		if ((nsendSize = write(i_connfd_2, packet.getBufferPtr(), packet.getBufferSize())) < 0)
		{
			printf("write Error: %s (errno: %d)\n", strerror(errno), errno);
			exit(0);
		}
